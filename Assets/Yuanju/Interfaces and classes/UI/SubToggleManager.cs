...
public class SubToggleManager : MonoBehaviour
{
    ...
    public void GetRandomTask()
    {
        ...
        if (gameObject.transform.parent.GetComponent<Toggle>().isOn && (PanelManager.listToggleText[1] == "Testing" || PanelManager.listToggleText[1] == "Verifica"))
        {
            ...
            subTaskSet.Clear();
            for (int i = 0; i < NPOIGetInitialFinalSettings.Tasks.Count; i++)
            {
                //Debug.Log("tasks:: " + NPOIGetInitialFinalSettings.Tasks[i]);
                if (gameObject.transform.parent.GetComponentInChildren<Text>().text == NPOIGetInitialFinalSettings.Tasks[i])
                {
                    subTaskSet.Add(NPOIGetInitialFinalSettings.SubTasks[i]);
                    //Debug.Log("sub tasks:: " + NPOIGetInitialFinalSettings.SubTasks[i]);
                }
            }
