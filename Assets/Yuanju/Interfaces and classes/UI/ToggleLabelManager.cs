...
public class ToggleLabelManager : MonoBehaviour
{
    ...
    void InitializeToggleLabels()
    {
        ...
        for (int i = 0; i < PanelManager.allToggles.Length; i++)
        {
            ...
            for (int j = 0; j < NPOIGetInitialFinalSettings.SubTasks.Count; j++)
            {
                if (NPOIGetInitialFinalSettings.SubTasks[j].Contains(PanelManager.allToggles[i].GetComponentInChildren<Text>(true).text) )
                {
                    PanelManager.allToggles[i].labelDetailed = NPOIGetInitialFinalSettings.GeneratorStatus[j];
                }
            }
        }
    ...
    {
        problemText.GetComponentInChildren<Text>().text = NPOIGetInitialFinalSettings.GeneratorStatus[NPOIGetInitialFinalSettings.SubTasks.IndexOf(PanelManager.listToggleText[2])];
        backGroundPanel.SetActive(true);
        HelpMenuButtons.IsProblemButtonClicked = true;
    }
    ...
    public static void HideProblem()
    {
        ...
        HelpMenuButtons.IsProblemButtonClicked = false;
