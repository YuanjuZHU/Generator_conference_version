...
public class Evaluation : MonoBehaviour
{
    ...
    private void CheckSequence()
    {
        ...
        Debug.Log("NPOIGetDatatable.SequenceDatatables.Count in evaluation: " + NPOIGetSequenceTable.SequenceDatatables.Count);
        ...
        {
            if (NPOIGetSequenceTable.SequenceDatatables[i].TableName.Contains(PanelManager.listToggleText[2])) //find the corresponding data table by comparing the text of the selected task toggle with the task in the sequence data tables
            {
                var dt = NPOIGetSequenceTable.SequenceDatatables[i];
                var drs = dt.Select();
    ...
    public static void CheckFinalSettings()
    {
        ...
        for (int i = 0; i < NPOIGetInitialFinalSettings.FinalSettings.Count; i++)
        {
            if (NPOIGetInitialFinalSettings.FinalSettings[i].TableName.Contains(PanelManager.listToggleText[2]))
            {
                finalSettingdt = NPOIGetInitialFinalSettings.FinalSettings[i].Copy();  //find the final setting data table that will be used for the checking. It is not necessary to use the copy of the data table
            }
        }

        var names = NPOIGetInitialFinalSettings.ActuatorNames; //only the actuators will be checked, the control elements will be checked in other way later
    ...
    private static void CheckGroupsStatusesFinalSetting(DataTable dt)
    {
        ...
        for (int j = 0; j < finalSettingGroupIndex.Count; j++)
        {
            ...
            for (int i = 0; i < NPOIGetInitialFinalSettings.ActuatorNames.Count; i++)
            {
                //Debug.Log("the elements of the Red start: " +  (StatusData)dr[columnTitle].Status);
                StatusData sd = (StatusData)finalSettingdt.Rows[0][NPOIGetInitialFinalSettings.ActuatorNames[i]]; //get the elements of a column 

                if (sd.Group == finalSettingGroupIndex[j])
                {
                    myList.Add(GameObject.Find(NPOIGetInitialFinalSettings.ActuatorNames[i]));
    ...
    public void ResetGame()
    {
        ...
        for(int i = 0; i < names.Count; i++)
        {
            ...
            var objectClass = GameObject.Find(NPOIGetSequenceTable.OperableComponentNames[i]).GetComponent<IGeneratorComponent>();
            objectClass.IsNeedCheck = false;
            if (GameObject.Find(NPOIGetSequenceTable.OperableComponentNames[i]).GetComponent<BlinkMaterial>()!=null)
            {
                var blink = GameObject.Find(NPOIGetSequenceTable.OperableComponentNames[i]).GetComponent<BlinkMaterial>();
                blink.IsBlink = false;
            }
    ...
    public static void GetComponentSequenceChecked(DataTable dt, string columnTitle)
    {
        ...
        for (int i = 0; i < NPOIGetSequenceTable.OperableComponentNames.Count; i++) //skip the "sequence" row, and the parameter rows
        {
            ...
            if(sd.Group == null) {
                ...
                if(sd.Status.HasValue) {
                    ...
                    {
                        if(SetScenes.ScriptComponents[NPOIGetSequenceTable.OperableComponentNames[i]].Status != (int)sd.Status)  //if the status of an actuator is not correct according to the sequence
                        {
                            ...
                            WrongAction wrongAction = new WrongAction();
                            wrongAction.Initialize();
                            wrongAction.WrongActuator = GameObject.Find(NPOIGetSequenceTable.OperableComponentNames[i]);
                            wrongAction.ExpectedStatus = (int)sd.Status;
                            wrongAction.Status = SetScenes.ScriptComponents[NPOIGetSequenceTable.OperableComponentNames[i]].Status;
    ...
    private static void CheckGroupsStatusesSequence(DataTable dt, string columnTitle)
    {
        ...
        for (int j = 0; j < sequenceGroupIndex.Count; j++)
        {
            ...
            for (int i = 0; i < NPOIGetSequenceTable.OperableComponentNames.Count; i++)
            {
                ...
                {
                    myList.Add(GameObject.Find(NPOIGetSequenceTable.OperableComponentNames[i]));
                    constraintSymbole = sd.Symbol;
                    constraintStatus = sd.Status;
                }
            }
    ...
    private void CreateSequence()
    {
        ...
        for (int i = 0; i < NPOIGetSequenceTable.SequenceDatatables.Count; i++)
        {
            Debug.Log("sequence tables: " + NPOIGetSequenceTable.SequenceDatatables[i]);
