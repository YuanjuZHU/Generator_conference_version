...
public class WaterLevelIndicator : MonoBehaviour, IGeneratorComponent
{
    ...
    public float StatusStartValue; //the start water level, unit millimeter
    public float StatusEndValue; //maximun water level value that this indicator can show
    public float PumpStartLevel;
    public float PumpStopLevel;
    ...
    public void UpdateMaterials() //in this class, update materials is used to update the level of water in the indicator 
    {
        ...
        waterLevel.transform.localScale = new Vector3(originalLocalScale.x, originalLocalScale.y, (scale * (status-StatusStartValue)/100f));
    }
