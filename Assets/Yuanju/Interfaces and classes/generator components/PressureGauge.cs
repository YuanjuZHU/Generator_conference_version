...
public class PressureGauge : MonoBehaviour, IGeneratorComponent
{
    ...
    {
        pressureNeedle.transform.localEulerAngles = new Vector3(originalRotation.x, originalRotation.y-status * (float)0.00018, originalRotation.z); //18 = 90 / 5 which is got from the picture in the slides. 
                                                                                                                    // using Pascal unit, we need to use a factor of 90° / (5*10^5) ==> 0.00018
    }
