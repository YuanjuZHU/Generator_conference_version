...
public class SetScenes : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            for (int i = 0; i < NPOIGetInitialFinalSettings.SubTasks.Count; i++)
            {
                //TODO needs to implement for the italian version
                if (NPOIGetInitialFinalSettings.SubTasks[i].Contains(PanelManager.listToggleText[2]))
                {
                    Debug.Log("the task in the excel: " + NPOIGetInitialFinalSettings.SubTasks[i]);
                    Debug.Log("the task in the toggle: " + PanelManager.listToggleText[2]);
                    PopSettings(NPOIGetInitialFinalSettings.SubTasks[i]);
                }
            }
    ...
    private void PopSettings(string task)
    {
        foreach (var Name in GeneratorElements)
        {
            ...
            ScriptComponents[Name].Status = NPOIGetInitialFinalSettings.TaskComponentInitialSettings[task][Name];
